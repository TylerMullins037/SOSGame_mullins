...
public class SOSReplay {
    private SOSGame game;
    private List<SOSRecorder.MoveRecord> moves;
    ...
    public interface ReplayListener {
        void onMovePlayed(SOSRecorder.MoveRecord move, int currentIndex, int totalMoves);
        void onReplayComplete();
        void onReplayError(String message);
    }
    
    // Constructor
    public SOSReplay(SOSRecorder.GameRecord gameRecord, List<SOSRecorder.MoveRecord> moves, ReplayListener listener) {
    ...
    public boolean stepBackward() {
        if (currentMoveIndex > 0) {
            ...
            for (int i = 0; i < currentMoveIndex - 1; i++) {
                SOSRecorder.MoveRecord move = moves.get(i);
    ...
    // Play a single move
    private void playMove(SOSRecorder.MoveRecord move) {
        if (move.getPlayer() == 'B') {
    ...
    private void makeMoveSilently(SOSRecorder.MoveRecord move) {
        if (move.getPlayer() == 'B') {
            game.setBlue(move.getChoice());
        } else {
            game.setRed(move.getChoice());
