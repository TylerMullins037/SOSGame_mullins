...
public abstract class SOSGame {
    ...
    public void startRecording(String bluePlayerType, String redPlayerType) {
        ...
        if (gameId != -1) {
            ...
            System.err.println("Failed to start game recording");
        }
    }
    
    // Stop recording the current game
    public void stopRecording() {
        if (recorder != null && recordingEnabled) {
            recorder.endGameRecording();
            recordingEnabled = false;
        }
    }
    
    // Record a move
    protected void recordMove(char player, int row, int col, char choice, boolean sosFormed) {
        if (recordingEnabled && recorder != null) {
            recorder.recordMove(player, row, col, choice, ++moveCount, sosFormed);
        }
    }
